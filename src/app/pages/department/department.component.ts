    ...
    <div class="section-card">
      <div class="section-header">
        <div class="section-title">
          <span class="material-icons-round text-primary">domain</span>
          Quản lý phòng ban
        </div>
      </div>
      ...
      <div class="toolbar mb-4" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <button class="btn-primary" (click)="openAddModal()">
          <span class="material-icons-round">add</span> Thêm Phòng Ban
        </button>
        <app-search-filter
      ...
      <div class="table-responsive">
        <table class="custom-table">
          ...
          <tbody>
            @for (dept of departments; track dept.departmentId; let i = $index) {
              <tr>
                <td>{{ (currentPage - 1) * pageSize + i + 1 }}</td>
                <td class="font-weight-600">{{ dept.departmentCode }}</td>
                <td class="font-weight-600 text-primary">{{ dept.departmentName }}</td>
                <td>{{ dept.managerName || '-' }}</td>
                <td>{{ dept.phone || '-' }}</td>
                <td class="text-muted">{{ dept.description || '-' }}</td>
                <td>
                  <app-status-badge [status]="dept.isActive ? 'Hoạt động' : 'Ngừng hoạt động'"></app-status-badge>
                </td>
                <td>
                  <div class="td-actions justify-end">
                    <button class="btn-icon-action text-info" (click)="openEditModal(dept)" title="Sửa thông tin">
                      <span class="material-icons-round">edit</span>
                    </button>
                    <button class="btn-icon-action text-danger" (click)="deleteDepartment(dept.departmentId)" title="Xóa phòng ban">
            ...
            @if (departments.length === 0) {
              <tr>
                <td colspan="8" class="text-center py-5 text-muted">
                  <span class="material-icons-round" style="font-size: 3rem; opacity: 0.2">domain_disabled</span>
                  <div class="mt-2">Không tìm thấy phòng ban nào...</div>
                </td>
              </tr>
            }
          </tbody>
      ...
      <div class="modal-overlay">
        <div class="modal-content" style="max-width: 500px;">
          <div class="modal-header">
            <h3>{{ isEditing ? 'Sửa Phòng Ban' : 'Thêm Phòng Ban' }}</h3>
          ...
          <form [formGroup]="deptForm" (ngSubmit)="onSubmit()">
            <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <input type="text" formControlName="departmentCode" class="form-control" [readonly]="isEditing">
                @if (deptForm.get('departmentCode')?.invalid && deptForm.get('departmentCode')?.touched) {
                  <div class="error-message">Mã phòng ban không được để trống (tối đa 20 ký tự).</div>
                }
              </div>
              ...
              <div class="mb-3">
                ...
                @if (deptForm.get('departmentName')?.invalid && deptForm.get('departmentName')?.touched) {
                  <div class="error-message">Tên phòng ban không được để trống (tối đa 100 ký tự).</div>
                }
              </div>
              ...
              <div class="mb-3" style="display: flex; align-items: center; gap: 8px;">
                ...
                <label for="isActiveCheck" style="margin: 0; cursor: pointer;">Phòng ban đang hoạt động</label>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn-outline" (click)="closeModal()">Hủy</button>
              <button type="submit" class="btn-primary" [disabled]="deptForm.invalid">Lưu dữ liệu</button>
            </div>
          </form>
        </div>
      </div>
...
export class DepartmentComponent implements OnInit {
  ...
  loadDepartments() {
    this.deptService.getPaged(this.currentPage, this.pageSize, this.searchQuery).subscribe({
      next: (res) => {
      ...
      error: () => this.notif.error('Không thể tải danh sách phòng ban')
    });
  }
  ...
  openAddModal() {
    ...
    this.deptForm.get('departmentCode')?.enable();
    this.showModal = true;
  }
  ...
  openEditModal(dept: Department) {
    ...
    this.deptForm.get('departmentCode')?.disable(); // Không cho phép sửa mã
  ...
  onSubmit() {
    ...
    const formData = this.deptForm.getRawValue();
    
    const request = this.isEditing 
      ? this.deptService.update(this.editingId!, formData)
      : this.deptService.create(formData);
    ...
    request.subscribe({
      next: (res) => {
        if (res.success) {
          this.closeModal();
          this.loadDepartments();
          this.notif.success(this.isEditing ? 'Cập nhật phòng ban thành công' : 'Thêm phòng ban thành công');
        }
      },
  ...
  deleteDepartment(id: number) {
    if (confirm('Bạn có chắc chắn muốn xóa phòng ban này?')) {
      this.deptService.delete(id).subscribe({
        next: (res) => {
          if (res.success) {
            this.notif.success('Đã xóa phòng ban');
            this.loadDepartments();
          }
        },
        error: (err: HttpErrorResponse) => {
          this.notif.error(`Lỗi Server: ${err.error?.message || 'Không thể xóa phòng ban'}`);
        }
      });
    }
  }
